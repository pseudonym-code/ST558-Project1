...
access_NHL_API <- function(extension = NULL, ID = NULL, ...){
  if(!is.numeric(ID) & !is.null(ID)){
    if(!is.null(which(team_mapping$fullName == ID))){
      i <- which(team_mapping$fullName == ID)
    } else if(!is.null(which(team_mapping$teamAbbrev == ID))){
      i <- which(team_mapping$teamAbbrev == ID)
    } else if(!is.null(which(team_mapping$teamCommonName == ID))){
      i <- which(team_mapping$teamAbbrev == ID)
    } else{
      stop("Team ID not found!")
    }
    ID <- team_mapping$id[i]
  }
